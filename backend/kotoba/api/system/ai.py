...
from kotoba.services.ai import client, pricing, prompts
...
def explain(body: ExplainIn, request: Request, db: Session = Depends(get_db)) -> dict:
    ...
    llm_client, model = client.client_from_settings(db, transport=transport)
    call = LlmCall(purpose="explain", model=model, ok=False)
    try:
        parsed, usage = llm_client.chat_json(system, user)
    except ApiError:
        db.add(call)
        db.commit()
        raise
    ...
    call.cost_estimate = pricing.estimate_cost(model, usage)
    call.ok = True
    db.add(call)
    parsed["_model"] = model
...
@router.get("/presets")
def presets() -> dict:
    return {"presets": pricing.PRESETS, "prices_usd_per_1m": pricing.PRICES}
