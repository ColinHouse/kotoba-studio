...
- **文案走 i18n 资源，标识符、注释、提交信息用英文。** `frontend/src/i18n/zh-CN.ts` 是**事实基准**：
  那里的中文就是界面上现有的中文，改文案改资源文件，不要在组件里写死；`en.ts` 按同一张 key 表翻译。
  key 按来源分组（`nav.*`、`settings.*`、`errors.*`），**不要用英文原文当 key**。默认语言是
  `zh-CN`，不跟随浏览器语言；选择存 `localStorage`。后端错误消息不在后端翻译：前端按
  `error.code` 映射（`stores/app.ts` 的 `fail()` 是唯一收口），中文下直接显示后端原文。
- **不要留下被注释掉的代码。** git 记得它。
