...
def test_explain_requires_key(client, monkeypatch):
    ...
    monkeypatch.setattr(llm, "get_api_key", lambda provider: (None, "none"))
...
def test_cost_estimate_uses_cache_hits():
    usage = llm.Usage(prompt_tokens=1_000_000, completion_tokens=0, cache_hit_tokens=1_000_000)
    assert llm.estimate_cost("deepseek-flash", usage) == pytest.approx(0.006)
    usage = llm.Usage(prompt_tokens=1_000_000, completion_tokens=1_000_000, cache_hit_tokens=0)
    assert llm.estimate_cost("deepseek-flash", usage) == pytest.approx(1.50)
