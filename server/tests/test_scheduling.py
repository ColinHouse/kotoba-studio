...
def test_scheduled_review_advances_due(client, db):
    ...
    fsrs_service.review(db, card, 3, mode="scheduled", now=now)
    db.commit()
...
def test_session_quiz_never_touches_fsrs_state(client, db):
    ...
    fsrs_service.review(db, card, 1, mode="session_quiz", now=now + timedelta(minutes=5))
    fsrs_service.review(db, card, 4, mode="session_quiz", now=now + timedelta(minutes=6))
    db.commit()
    after = (
        card.fsrs_state,
        card.fsrs_step,
...
def test_queue_respects_device_ownership(client, db):
    ...
    desktop_ids = {c.id for c in fsrs_service.queue(db, "desktop")}
    mobile_ids = {c.id for c in fsrs_service.queue(db, "mobile")}
