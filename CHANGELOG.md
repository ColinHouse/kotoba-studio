...
- 采集：剪贴板监听有了界面入口——开启后，任何把台词复制到剪贴板的工具（包括 Textractor 的
  复制功能）都会被自动接住，不需要给 Textractor 装 WebSocket 扩展。默认关闭。
- 覆盖层：拖过的位置跨重启保留，按作品各存一个——换游戏自动切回那个游戏的位置；没有绑定作品的
  会话共用一个全局位置。双击标题复位时会一并清掉存下来的位置，分辨率变化后位置会被夹回屏幕内。
- 采集：Hook 成为推荐的默认文本来源，OCR 保留为兜底——两者互不排斥，选哪条都随时能用另一条。
  采集页按偏好排序并标出推荐项，写明各自适用场景；Hook 未检测到时会指向屏幕识别。已有 OCR
  采集记录的用户保持原样，不会被引导打扰。
- 采集：OCR 识别结果现在看得见——截图上按文本块画框，每块标出文字与置信度（低于 0.6 用虚线框，
  不用颜色），点一下复制那一块的文字；上方显示引擎与耗时。只给整段文本、没有分块坐标的引擎
  退回纯文本并说明原因。
...
- 采集：绑定了游戏窗口之后，只要那个窗口不在最前面，采集就会暂停而不是照样截图。
  以前窗口读取失败会悄悄退回抓屏幕，于是盖在游戏上面的浏览器、聊天窗口都可能被
  当成台词收进收件箱——少收一句最多再按一次热键，收错一句却会污染词库、词频排序
  和复习调度。区域监视器同样会暂停，界面上能看到暂停原因。

### Fixed

- 采集：OCR 不再把覆盖层自己的面板读进台词。屏幕抓图（窗口抓不到时的回退、没绑窗口的区域）
  会在识别前把覆盖层所在的矩形涂黑；窗口抓图本来就不含覆盖层。此前覆盖层压在识别区域上时，
  它的标题和显示的上一句会被读回来，越积越乱。
...
- 备份：恢复时会先检查压缩包里每个文件的路径，任何指向媒体目录之外的成员都会让整个
  恢复中止。备份是可以在机器之间携带的文件，因此包里的路径属于不可信输入；此前它们
  被直接拼到媒体目录上，`..` 不会被规范化。自己生成的备份不受影响。

### Fixed

- 采集：OCR 把小假名读成大假名（`やっぱり` 读成 `やつばり`）、漏读浊音（`ください` 读成
  `くたさい`）时，入库前会做一次保守纠正——只有整段恰好切成一个词、项目里已导入的词典
  确有其词、且改动方向唯一时才改，拿不准就保持原样；`つ` 动词的促音便（`まつて` → `まって`）
  按语法规则纠正。Hook 与剪贴板文本不受影响，引擎原文仍保留在 `raw_text`。
...
- 界面：句子里「已掌握」的词块字重从 300 提到 400。这一档的对比度本来就贴着大字号的
  下限，细字重会让笔画在视觉上更淡；加了字重之后仍明显轻于未学词，浓淡分层不受影响。
  助词保持细字重——它本来就该退到背景里。
...
- 界面：小字号的标签、时间、计数、图例过淡，现在一律用中墨。四档墨色本来是给句子用的
  （句子是大字号，门槛更低），被借去写 11–13px 的小字后就不够看了；小字改用中墨之后，
  九个页面里除了赭石强调色以外，文字对比度全部达标。
- 界面：浅色主题最浅的一档墨色略微加深，让「已忽略 / 助词」在句子里也读得出来。
  深色主题不变；四档的浓淡次序照旧。
