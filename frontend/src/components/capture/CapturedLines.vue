...
<template>
  <section class="flex min-w-0 flex-col">
    ...
    <TransitionGroup tag="ul" name="list" class="relative m-0 mt-3 flex list-none flex-col p-0">
      ...
      >
        ...
        <div class="min-w-0">
          ...
          <p class="m-0 mt-px text-[10px] text-ink-70">
    ...
    <p v-if="!lines.length" class="m-0 text-[12px] leading-relaxed text-ink-70">
      收藏的句子会出现在这里；游戏结束后到收件箱统一整理。
    </p>
    <RouterLink v-else :to="inboxLink" class="btn-quiet mt-4 self-start">去收件箱整理 →</RouterLink>

    <div class="mt-auto border-t border-rule pt-[18px] text-[11px] leading-[1.9] text-ink-70">
