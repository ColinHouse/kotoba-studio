...
<template>
  <section class="lifted px-6 py-[22px]" @keydown="onKey">
    <header class="flex items-baseline justify-between gap-3 border-b border-divider pb-3">
      ...
      <span class="shrink-0 text-[11px] text-ink-70">
    ...
    <div class="mt-4 grid gap-[22px] md:grid-cols-2">
      <div v-if="picked.candidates.length">
        ...
        >
          <span class="flex flex-wrap items-baseline gap-2">
            ...
            <span
              class="jp text-[13px]"
              :class="candidate?.id === entry.id ? 'text-gold' : 'text-ink-70'"
              >{{ entry.reading }}</span
            >
      ...
      <p v-else class="m-0 text-[12px] text-ink-70">词典里没有候选，请手填词条与释义。</p>
      ...
      <div class="flex flex-col gap-3">
        ...
        <div>
          <label class="field-label" for="term-gloss-en">
            词典释义
            <span v-if="candidate" class="text-ink-70">· {{ candidate.dict_title }}</span>
          </label>
          <input id="term-gloss-en" v-model="glossEn" class="input" />
        </div>
        <div>
          <p class="mb-1.5 text-[12px] text-ink-70">建哪几张卡</p>
          <div class="seg">
            <button
    ...
    <footer class="mt-5 flex flex-wrap items-center gap-3 border-t border-rule pt-4">
      ...
      <span class="num ml-auto hidden text-[11px] text-ink-70 md:inline"
