...
<template>
  <div :class="bare ? '' : 'border-t border-rule bg-surface px-6 pt-4 pb-[18px]'">
    <div class="mb-2.5 flex items-baseline justify-between">
      <span class="kicker text-ink-70">AI 语境解释</span>
      <span v-if="confidence" class="num type-micro text-ink-70">{{ confidence }}</span>
    </div>
    <dl class="m-0 grid grid-cols-[62px_1fr] gap-x-3 gap-y-2 type-note leading-[1.65] text-ink-70">
      <template v-for="row in rows" :key="row.key">
        <dt class="text-right type-micro text-ink-70">{{ row.label }}</dt>
