  ...
  <div @keydown="onKey">
    <header class="flex flex-wrap items-center justify-between gap-4 border-b border-divider pb-3">
      <div class="flex items-baseline gap-3">
        ...
        <span v-if="session" class="inline-flex items-center gap-[7px] text-[12px] text-ink-70">
    ...
    <template v-else>
      ...
      <div
        class="flex flex-wrap items-center gap-x-[22px] gap-y-2 border-b border-rule py-2.5 text-[12px] text-ink-70"
      >
        ...
        <label class="flex items-center gap-1.5">
          ...
          <select
            ...
            class="num border-0 border-b border-divider bg-transparent text-[12px] text-ink-70"
          >
        ...
        <label class="flex items-center gap-1.5">
          ...
          <select
            ...
            class="border-0 border-b border-divider bg-transparent text-[12px] text-ink-70"
          >
        ...
        <label class="flex items-center gap-1.5">
          游戏窗口
          <select
            class="max-w-64 border-0 border-b border-divider bg-transparent text-[12px] text-ink-70"
      ...
      <div class="mt-5 md:grid md:grid-cols-[3fr_1px_2fr]">
        <div class="md:pr-[26px]">
          ...
          />
          <div v-else class="framed grid place-items-center p-10 text-[13px] text-ink-70">
            正在截取屏幕预览…
          </div>

          <div class="mt-2 flex items-baseline justify-between gap-3 text-[11px]">
            <span v-if="capture.region.value" class="num text-ink-70">
          ...
          <div class="mt-2.5 flex flex-wrap items-center gap-[18px] text-[12px] text-ink-70">
            <button class="btn-quiet" :disabled="!framed || busy" @click="capture.runOcr">
              {{ capture.busy.value === 'ocr' ? '识别中…' : '只识别，不保存' }}
            </button>
            <span class="text-ink-70">按一次就回游戏；识别结果在右边核对。</span>
          </div>
        </div>
