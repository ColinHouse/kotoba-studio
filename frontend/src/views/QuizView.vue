  ...
  <div class="mx-auto max-w-[660px]">
    ...
    <Transition name="rise" mode="out-in">
      <div v-if="!loading && current">
        ...
        <p class="mt-2 mb-0 type-note text-ink-70">{{ current.hint }}</p>
        ...
        <div v-else class="mt-[22px] border-t border-rule pt-[18px]">
          <dl class="m-0 grid grid-cols-[64px_1fr] items-baseline gap-x-3.5 gap-y-2.5">
            <template v-if="current.kind !== 'meaning'">
              <dt class="text-right type-micro tracking-[0.1em] text-ink-70">你写的</dt>
            ...
            <dt class="text-right type-micro tracking-[0.1em] text-ink-70">
          ...
          <div class="mt-5 flex flex-wrap items-center gap-3.5">
            ...
            <span class="type-micro text-ink-70">短测只作记录，不改变 FSRS 的正式安排。</span>
          </div>
        </div>
      </div>
      ...
      <section v-else-if="!loading && summary" class="mt-6">
        <div class="border-b border-divider pb-[18px] text-center">
          ...
          <p class="num m-0 type-meta text-ink-70">时长 {{ fmtDuration(summary.duration_s) }}</p>
        </div>
        ...
        <div class="mt-[22px] grid grid-cols-4">
          ...
          >
            ...
            <p class="m-0 mt-1.5 type-micro tracking-[0.1em] text-ink-70">{{ stat.label }}</p>
          </div>
        </div>
