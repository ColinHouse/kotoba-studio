...
<template>
  <div class="flex min-h-[calc(100dvh-160px)] flex-col md:min-h-0">
    <header class="flex items-baseline justify-between gap-3">
      ...
      <span class="num type-meta text-ink-50 md:type-note">
    ...
    <p v-if="offline" class="mt-3 mb-0 type-micro text-ink-50">
      当前无法连接服务器，正在用上次取到的卡片复习；记录会保存在本机，联网后自动同步。
    </p>

    <p v-if="loading" class="mt-10 text-center type-note text-ink-35">加载中…</p>
    ...
    <template v-else-if="current">
        ...
        <div class="flex-1">
          <Transition name="fade" mode="out-in">
            <button
              ...
              @click="revealed = true"
            >
              显示答案<span class="type-meta tracking-wider text-ink-50">空格</span>
            </button>
            <RatingBar v-else key="rating" :preview="current.preview" @rate="rate" />
          </Transition>
        </div>
        <div
          class="hidden w-[190px] shrink-0 border-l border-divider pl-4 type-micro leading-[1.9] text-ink-35 md:block"
        >
      ...
      <p class="mt-2.5 mb-0 text-center type-micro text-ink-35">
    ...
    <Transition name="rise">
      <div v-if="!loading && !current" class="mt-8">
        ...
        <p class="mt-2 mb-0 type-note leading-relaxed text-ink-50">
