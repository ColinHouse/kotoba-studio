...
<template>
  <div>
    <header class="mb-5">
      ...
      <p class="mt-1 text-[12px] text-ink-50">
        常用汉字表按掌握度着色，墨色越深越熟；点一个字看它出现在哪些词里。
      </p>
    </header>

    <div v-if="grid && !grid.installed" class="border border-rule p-4">
      <p class="m-0 text-[13px] text-ink">还没有导入 KANJIDIC2 汉字表（EDRDG，CC BY-SA 4.0）。</p>
      <button class="btn btn-primary mt-3" :disabled="installing" @click="install">
        导入汉字表
      </button>
      <p v-if="installMessage" class="mt-2 text-[12px] text-ink-50">{{ installMessage }}</p>
    </div>

    <template v-else-if="grid">
      <p class="num mb-3 text-[12px] text-ink-50">
      ...
      <section v-if="selected" class="mt-6 border-t border-rule pt-4">
        <h2 class="m-0 font-head text-[18px] text-ink">
          ...
          <span class="ml-2 text-[12px] text-ink-50">{{ terms.length }} 个词</span>
        </h2>
        <ul v-if="terms.length" class="mt-2 list-none p-0">
          <li
            v-for="term in terms"
            :key="term.term_id"
            class="border-b border-rule py-1.5 text-[14px]"
          >
            <RouterLink :to="`/terms/${term.term_id}`" class="no-underline">
              <span class="jp text-ink">{{ term.headword }}</span>
              <span class="jp ml-2 text-[12px] text-ink-50">{{ term.reading }}</span>
            </RouterLink>
            <span v-if="term.has_card" class="ml-2 text-[11px] text-gold">有卡</span>
            <span v-else-if="term.known_status === 'known'" class="ml-2 text-[11px] text-ink-50"
              >已知</span
            >
          </li>
        </ul>
        <p v-else class="mt-2 text-[13px] text-ink-35">还没有含这个字的词。</p>
      </section>

      <section class="mt-6 border-t border-rule pt-4">
        <h2 class="m-0 font-head text-[16px] text-ink">作品里的超纲字</h2>
        <p class="mt-1 text-[12px] text-ink-50">超出常用汉字表的字，按在这部作品里的出现次数排。</p>
        <select
          v-model="sourceId"
          class="mt-2 border border-rule bg-transparent px-2 py-1 text-[13px] text-ink"
          @change="loadSource"
        >
        ...
        <ul v-if="sourceKanji.length" class="mt-2 flex list-none flex-wrap gap-2 p-0">
          <li v-for="item in sourceKanji" :key="item.character">
            <button
              type="button"
              class="jp cursor-pointer border border-rule bg-transparent px-2 py-1 text-[15px] text-ink"
              :title="`出现 ${item.occurrences} 次，${item.terms} 个词`"
              @click="pick(item.character)"
            >
              {{ item.character
              }}<span class="num ml-1 text-[11px] text-ink-50">{{ item.occurrences }}</span>
            </button>
          </li>
        </ul>
        <p v-else-if="sourceId" class="mt-2 text-[13px] text-ink-35">
