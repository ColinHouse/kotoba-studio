...
<template>
  <div>
    <header class="mb-5">
      ...
      <p class="mt-1 text-[12px] text-ink-70">
    ...
    <div v-if="grid && !grid.installed" class="border border-rule p-4">
      ...
      <p v-if="installMessage" class="mt-2 text-[12px] text-ink-70">{{ installMessage }}</p>
    </div>

    <template v-else-if="grid">
      <p class="num mb-3 text-[12px] text-ink-70">
      ...
      <section v-if="selected" class="mt-6 border-t border-rule pt-4">
        <h2 class="m-0 font-head text-[18px] text-ink">
          ...
          <span class="ml-2 text-[12px] text-ink-70">{{ terms.length }} 个词</span>
        </h2>
        <ul v-if="terms.length" class="mt-2 list-none p-0">
          ...
          >
            <RouterLink :to="`/terms/${term.term_id}`" class="no-underline">
              ...
              <span class="jp ml-2 text-[12px] text-ink-70">{{ term.reading }}</span>
            </RouterLink>
            <span v-if="term.has_card" class="ml-2 text-[11px] text-gold">有卡</span>
            <span v-else-if="term.known_status === 'known'" class="ml-2 text-[11px] text-ink-70"
              >已知</span
            >
          </li>
        </ul>
        <p v-else class="mt-2 text-[13px] text-ink-70">还没有含这个字的词。</p>
      </section>

      <section class="mt-6 border-t border-rule pt-4">
        <h2 class="m-0 font-head text-[16px] text-ink">作品里的超纲字</h2>
        <p class="mt-1 text-[12px] text-ink-70">超出常用汉字表的字，按在这部作品里的出现次数排。</p>
        <select
          v-model="sourceId"
        ...
        <ul v-if="sourceKanji.length" class="mt-2 flex list-none flex-wrap gap-2 p-0">
          <li v-for="item in sourceKanji" :key="item.character">
            ...
            >
              {{ item.character
              }}<span class="num ml-1 text-[11px] text-ink-70">{{ item.occurrences }}</span>
            </button>
          </li>
        </ul>
        <p v-else-if="sourceId" class="mt-2 text-[13px] text-ink-70">
