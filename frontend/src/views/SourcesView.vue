...
<template>
  <div>
    <header class="border-b border-divider pb-3.5">
      ...
      <p class="mt-0.5 mb-0 text-[13px] text-ink-70">
      ...
      <li v-for="s in sources" :key="s.id" class="border-b border-rule py-3.5">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div class="min-w-0">
            <p class="m-0 font-head text-[19px]">
              {{ s.title }}
              <span v-if="s.title_ja" class="jp text-[13px] font-normal text-ink-70">{{
                s.title_ja
              }}</span>
            </p>
            <p class="num m-0 text-[11px] text-ink-70">
        ...
        <Transition name="rise">
          <div v-if="coverageId === s.id" class="framed mt-3 p-4">
            <p v-if="coverageBusy || !coverage" class="m-0 text-[13px] text-ink-70">正在统计…</p>
            <template v-else>
              ...
              <div v-if="coverage.unknown_top.length" class="flex flex-wrap gap-x-4 gap-y-2">
                ...
                >
                  ...
                  <span class="num text-[11px] text-ink-70">×{{ w.count }}</span>
                </RouterLink>
              </div>
              <p v-else class="mt-3 mb-0 text-[13px] text-ink-70">这个作品暂时没有未学的词。</p>
              ...
              <p v-if="!coverage.has_frequency" class="mt-3 mb-0 text-[11px] text-ink-70">
    ...
    <p v-if="!sources.length" class="py-4 text-[13px] text-ink-70">
