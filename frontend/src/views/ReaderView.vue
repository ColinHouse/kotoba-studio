  ...
  <div>
    <header class="flex flex-wrap items-end justify-between gap-3 border-b border-divider pb-3">
      ...
      <div class="flex items-center gap-4">
        ...
        <p v-if="current" class="num m-0 text-[13px] text-ink-70">
          第 {{ current.page }} / {{ pages.length }} 页
        </p>
      </div>
    </header>

    <p v-if="loading" class="mt-6 m-0 text-[13px] text-ink-70">正在打开…</p>
    <p v-else-if="!pages.length" class="mt-6 m-0 text-[13px] text-ink-70">
    ...
    <div v-else-if="current" class="mt-4 lg:grid lg:grid-cols-[minmax(0,1fr)_360px] lg:gap-6">
      <section class="min-w-0">
        ...
        <div class="mt-3 flex items-center justify-between gap-3">
          ...
          <span class="hidden text-[11px] text-ink-70 md:inline">← → 翻页 · 双指缩放</span>
          <button
      ...
      <aside class="mt-4 lg:mt-0">
        <template v-if="selected">
          ...
          >
            ...
            <p v-else class="mt-4 m-0 text-[12px] text-ink-70">正在分词…</p>
        ...
        <p v-else class="mt-1 text-[13px] text-ink-70 lg:mt-0">
