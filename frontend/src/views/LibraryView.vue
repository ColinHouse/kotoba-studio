...
function wordClass(t: Term) {
  switch (t.known_status) {
    case 'known':
      return 'font-light text-ink-70'
    case 'learning':
      return 'font-normal text-ink-70'
    case 'ignored':
      return 'font-light text-ink-70 line-through'
    default:
...
<template>
  <div>
    <header class="flex flex-wrap items-end justify-between gap-5 border-b border-divider pb-3.5">
      ...
      <div class="flex flex-wrap items-center gap-3.5">
        <input
          id="library-search"
          v-model="q"
          class="jp w-[200px] border-0 border-b border-divider bg-transparent pb-[5px] type-body text-ink outline-none placeholder:text-ink-70 focus-visible:border-accent md:w-[260px]"
          placeholder="搜索词或读音…"
        />
        <div class="seg">
          <button
        ...
        <span v-else class="type-micro text-ink-70" title="在设置页导入 Yomitan 频率词典后可用">
          频率排序需先导入频率词典
        </span>
      </div>
    </header>

    <p v-if="route.query.source" class="mt-3 mb-0 type-micro text-ink-70">
    ...
    <Transition name="rise" mode="out-in">
        ...
        <ul class="m-0 mt-2 flex list-none flex-col p-0">
          <li v-for="t in terms" :key="t.id" :class="t.trap ? 'bg-accent-100' : ''">
            ...
            >
              ...
              <span
                class="min-w-0 flex-1 type-body md:type-body"
                :class="
                  t.trap
                    ? 'text-accent-800'
              ...
              >
                {{ glossOf(t) }}
                <span v-if="t.trap" class="type-meta text-gold"
                  >— 不是「{{ t.trap.zh_reading_meaning }}」</span
                >
              </span>
              <span class="num w-[96px] shrink-0 type-micro text-ink-70 md:text-right">
      ...
      <p v-else-if="!loading && !terms.length" class="mt-5 type-note text-ink-70">
