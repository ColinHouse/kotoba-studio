  ...
  <div v-if="term" class="mx-auto max-w-4xl space-y-6">
    ...
    <header class="card p-5">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p class="jp text-lg text-accent-2">
            {{ term.reading }} <span class="text-sm text-ink-3">{{ term.pos ?? '' }}</span>
          </p>
        </div>
        <div class="flex flex-wrap gap-1">
          <button
            ...
            :class="{ 'ring-2 ring-accent': term.known_status === k }"
            @click="setStatus(k as KnownStatus)"
          >
            {{ label }}
          </button>
        </div>
      ...
      <ul class="mt-3 space-y-1">
        <li v-for="s in term.senses" :key="s.id" class="text-base">
          {{ s.gloss_zh || s.gloss_en }} <span class="text-xs text-ink-3">{{ s.origin }}</span>
        </li>
      </ul>
      <div
        v-if="term.trap"
        class="mt-3 rounded-xl border border-accent/30 bg-accent/10 p-3 text-sm"
      >
        <span class="label text-accent-2">中日同形异义</span>
        <p>
          中文「{{ term.trap.headword }}」＝{{ term.trap.zh_reading_meaning }}；日语＝<b>{{
            term.trap.ja_meaning
          }}</b
          >。{{ term.trap.note }}
        </p>
      </div>
      <p class="mt-3 text-xs text-ink-3">
        ...
        {{ term.card_count }} 张卡 · <StatusBadge :status="term.known_status" />
      </p>
    </header>
      ...
      <ol class="space-y-3">
        <li v-for="enc in term.encounters" :key="enc.id" class="card p-4">
          <div class="flex flex-wrap items-center justify-between gap-2 text-xs text-ink-3">
            ...
            <span v-if="enc.contraction_of"
              >缩约形「{{ enc.surface }}」← {{ enc.contraction_of }}</span
            >
          </div>
          ...
          <div class="mt-2 flex flex-col gap-3 md:flex-row">
            <img
              ...
              class="max-h-40 rounded-lg border border-line md:w-64 md:object-cover"
              alt="截图"
            />
    ...
    <section>
      ...
      <ul class="card divide-y divide-line">
        <li
          v-for="c in term.cards"
          :key="c.id"
          class="flex flex-wrap items-center justify-between gap-2 px-4 py-2 text-sm"
        >
          <span
            >{{ CARD_TYPE_LABEL[c.card_type] }}卡 ·
            {{ c.due ? `到期 ${humanInterval(c.due)}后` : '新卡'
            }}<span v-if="c.stability"> · 稳定性 {{ c.stability.toFixed(1) }} 天</span></span
          >
          <span class="flex items-center gap-1 text-xs">
            归属
            <button
              v-for="o in ['desktop', 'mobile', 'any'] as Owner[]"
              :key="o"
              class="btn-ghost px-2 py-1 text-xs"
              :class="{ 'ring-2 ring-accent': c.review_owner === o }"
              @click="setOwner(c, o)"
            >
              {{ { desktop: '电脑', mobile: '手机', any: '任意' }[o] }}
            </button>
          </span>
        </li>
        <li v-if="!term.cards.length" class="px-4 py-3 text-sm text-ink-2">
