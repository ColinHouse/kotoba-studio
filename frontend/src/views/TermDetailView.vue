  ...
  <div v-if="term">
    ...
    >
      <div class="min-w-0">
        ...
        <div class="mt-0.5 flex flex-wrap items-baseline gap-3">
          ...
          <span v-if="term.pos" class="type-meta text-ink-70">{{ term.pos }}</span>
        </div>
        <p v-if="term.senses.some((s) => s.gloss_en)" class="mt-1.5 mb-0 type-note text-ink-70">
          <template v-for="(s, i) in term.senses.filter((x) => x.gloss_en)" :key="s.id">
            <span v-if="i" class="text-ink-70">; </span>{{ s.gloss_en }}
            <span class="type-micro text-ink-70">{{ originLabel(s.origin) }}</span>
          </template>
        </p>
      </div>
      <div class="shrink-0 md:text-right">
        ...
        <p class="num mt-2.5 mb-0 type-micro text-ink-70">
    ...
    <section class="mt-[26px]">
      <div class="flex flex-wrap items-baseline gap-3">
        ...
        <span v-if="term.encounters.length" class="type-meta text-ink-70">
        ...
        <li v-for="(enc, i) in [...term.encounters].reverse()" :key="enc.id" class="tl-item">
          ...
          <div class="flex flex-col gap-4 md:flex-row md:gap-5">
            ...
            <div class="min-w-0">
              ...
              <p v-if="enc.contraction_of" class="m-0 type-meta text-ink-70">
    ...
    <section class="mt-[30px]">
      ...
      <p v-else class="m-0 type-note text-ink-70">还没有卡片。在收件箱确认这个词时勾选卡片类型。</p>
    </section>
  </div>
</template>
