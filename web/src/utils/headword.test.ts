...
const entry = (kanji: string[], kana: string[], uk: boolean): DictEntry => ({
  id: '1', dict_id: 1, kanji, kana, senses: [], pos: [], common: true, is_expression: false, usually_kana: uk,
  headword: kanji[0] ?? kana[0]!, reading: kana[0] ?? '',
})
...
describe('headwordFor', () => {
  ...
  it('prefers kana for usually-kana entries written in kana', () => {
    expect(headwordFor(entry(['仕様がない'], ['しょうがない'], true), 'しょうがない', 'x')).toBe('しょうがない')
  })
